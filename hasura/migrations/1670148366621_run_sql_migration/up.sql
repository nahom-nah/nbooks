CREATE OR REPLACE VIEW "public"."distributor_target_value_view" AS 
 SELECT sub.distributorid,
    ...
    ( SELECT sum(t.amount) AS sum
          ...
          WHERE ((r.slug = 'Coordinator'::text) AND (t.distributorid = sub.distributorid) AND ((t.created_at <= sub.end_date) AND (t.created_at >= sub.start_date)))) AS value
   FROM ( SELECT d.id AS distributorid,
           ...
           FROM ((distributor d
             JOIN target_outlet_view tr ON ((tr.distributorid = d.id)))
